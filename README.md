...
Predicting Billboard Hot 100 chart entry and longevity from Spotify audio features using logistic regression, XGBoost, Cox proportional hazards, and OLS. Dataset: 78,390 Spotify tracks, 2,157 charted (2.75% positive rate).
...
│
├── oit367_base_dataset.csv      ← Base dataset (89,741 tracks; input to build_final_dataset)
...
| `merged_spotify_billboard_data.csv` | Billboard source used by pipeline (has `track_id`); may be from Kaggle or built from spotify + hot-100 | ~20MB |

> `oit367_final_dataset.csv` is committed — it's the cross-ID deduped dataset that `run_all_v5.py` reads. To rebuild: run `build_final_dataset.py` first (requires `oit367_base_dataset.csv`, `augmented_deduped_dataset_with_artists.csv`).
...
To rebuild the final dataset from source:
```bash
python3 build_final_dataset.py   # Requires: oit367_base_dataset.csv, augmented_deduped_dataset_with_artists.csv
```
...
If `artist_features.csv` is present in the folder, it's auto-detected and merged as augmented features. Download from Modal after the scraper completes (see §7).
...
## 7. Cloud Data Augmentation (Modal)
...
| File | Description |
|---|---|
| `fig1_class_balance.png` | Class distribution (2.75% charted) |
| `fig2_correlation_heatmap.png` | Feature correlation matrix |
| `fig3_roc_curves.png` | ROC: LR (0.91) vs XGBoost (0.97) |
...
| `fig9_precision_recall.png` | PR curves (primary metric at 2.75% positive rate) |
...
## 9. Team Task Assignments

| Task | Owner | Status |
|---|---|---|
| Model pipeline (LR, XGBoost, Cox PH) | Alex / Vivian | ✅ Complete (v6) |
| SHAP analysis | Alex | ✅ Complete |
| Lyric sentiment (VADER) | Alex | ✅ Complete |
...
| Cox PH longevity analysis | Ben / Vivian | ✅ Complete (v6) |
